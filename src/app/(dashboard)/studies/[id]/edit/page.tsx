    ...
    async function fetchStudy() {
      try {
        ...
        setFormData({
          ...
          cappedDose: study.patientConstraints?.capped_dose ?? false,
          cappedDoseCondition: study.patientConstraints?.capped_dose_condition || '',
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1 }}>
              {formData.arms.map((arm, index) =>
                editingArmIndex === index ? (
                  ...
                  <Chip key={`arm-${index}`} label={arm} onClick={() => startEditArm(index)} onDelete={() => removeArm(index)} />
                )
              )}
            </Box>
