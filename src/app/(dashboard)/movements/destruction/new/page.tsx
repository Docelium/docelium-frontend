  ...
  useEffect(() => {
    if (formData.studyId && formData.medicationId) {
      fetch(`/api/stock?studyId=${formData.studyId}&medicationId=${formData.medicationId}`)
        .then((res) => res.json())
        .then((result) => setStockItems((result.data || []).filter((i: StockItem) => i.currentQuantity > 0)))
        .catch(console.error);
    }
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Enregistrer une destruction de medicaments
      </Typography>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            <Grid container spacing={3} sx={{ maxWidth: '100%' }}>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <FormControl fullWidth required>
                  ...
                  <Select
                    ...
                    onChange={(e) => handleChange('destructionMethod')(e as { target: { value: unknown } })}
                  >
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  fullWidth
                  label="Nom du temoin"
                  value={formData.destructionWitnessName}
                  onChange={handleChange('destructionWitnessName')}
                />
              </Grid>
              <Grid size={{ xs: 12, md: 6 }}>
                <TextField
                  fullWidth
                  label="Numero de certificat"
                  value={formData.destructionCertificateNumber}
                  onChange={handleChange('destructionCertificateNumber')}
                />
              </Grid>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 4, gap: 2 }}>
              ...
              <Button type="submit" variant="contained" color="error" disabled={loading}>
                {loading ? <CircularProgress size={24} /> : 'Detruire'}
              </Button>
            </Box>
          </Box>
        </CardContent>
